...
want_ssl=yes
AC_ARG_ENABLE(ssh,
    [  --disable-ssh           disable SSL],
    want_ssl=$enableval, want_ssl=yes)
AC_MSG_CHECKING(for OpenSSL)
...
if test "$ac_ssl_includes" = NO || test "$ac_ssl_libraries" = NO; then
  CXXFLAGS="$CXXFLAGS -D_NO_SSH_COMPILED"
  OPENSSL_INC=
  OPENSSL_LIB=
  LIB_OPENSSL=
  want_ssl=no
  AC_MSG_RESULT(no)
  AC_MSG_WARN([OpenSSL cannot be found on your system, SSH support will not be compiled, Check your setting and try to use --with-ssl-dir to specify the dir of your OpenSSL installation])
else
  CXXFLAGS="$CXXFLAGS -I$ssl_incdir $all_includes"
  OPENSSL_INC="-I$ssl_incdir"
  OPENSSL_LIB="-L$ssl_libdir"
  LIB_OPENSSL="-lssl"
  AC_MSG_RESULT(yes)
fi
  ...
  OPENSSL_LIB=
  LIB_OPENSSL=
  DO_NOT_COMPILE="$DO_NOT_COMPILE ssh"
  AC_MSG_RESULT(no)
  AC_MSG_WARN([SSH support disabled by the user])
fi

AM_CONDITIONAL(WANT_SSL, test x$want_ssl = xyes)
...
AC_ARG_WITH(esddir,
[  --with-esd-dir=DIR   where EsounD is installed (optional) ],
[
  esd_default_dirs="/usr/bin /usr/local/bin $withval"
...
AC_ARG_ENABLE(esd,
    ...
    want_esd=$enableval, want_esd=no)
if test $want_esd = yes; then
...
AC_ARG_WITH(artsdir,
[  --with-arts-dir=DIR   where aRts is installed (optional) ],
[
  arts_default_dirs="/usr /usr/local /opt/kde /opt/kde3 $withval"
...
AC_ARG_ENABLE(arts,
    ...
    want_arts=$enableval, want_arts=no)
if test $want_arts = yes; then
